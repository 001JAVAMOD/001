package com.mc1124.lightanddark.event;
...
@Mod.EventBusSubscriber(modid = "lightanddarkmod", bus = Mod.EventBusSubscriber.Bus.FORGE)
public class RubyArmorEffectHandler {
    ...
    private static int getRubyArmorCount(Player player) {
        ...
        for (ItemStack armor : player.getArmorSlots()) {
            if (armor.getItem() == RubyArmor.RUBY_HELMET.get() ||
                armor.getItem() == RubyArmor.RUBY_CHESTPLATE.get() ||
                armor.getItem() == RubyArmor.RUBY_LEGGINGS.get() ||
                armor.getItem() == RubyArmor.RUBY_BOOTS.get()) {
                count++;
            }
        }
        return count;
